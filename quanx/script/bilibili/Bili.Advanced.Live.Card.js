const $ = new Env("添加/删除卡片V0.0.9");


let cards = $.getjson("@Bili.Advanced.Live.Cards");
if (!cards) {
  $.log("\n🔴未获取到卡片缓存");
  $.setdata(JSON.stringify([]), "@Bili.Advanced.Live.Cards");
  cards = [];
};

(async function () {
  $.log($request.url)
  // get type of url add or remove
  const type = $request.url.includes('add.card') ? 'add' : 'remove';
  $.log(`type: ${type}`)
  // get roomID from the url
  const roomID = $request.url.match(/\/(\d+)\?/)[1];
  $.log(`roomID: ${roomID}`)
  // roomID = String(roomID)
  if (type == "add") {
    // 先判断数组里面是不是已经有这个id了再操作
    if (cards.includes(roomID)) {
      $.log(`\n🔴房间号 ${roomID} 已存在`)
    } else {
      cards.push(roomID)
    }
    $.setdata(JSON.stringify(cards), "@Bili.Advanced.Live.Cards")
    $.log(`\n🟢房间号 ${roomID} 已添加`)
    $.done({ body: JSON.stringify({ "Status": "Add Success  ", "Current Cards": cards }), encoding: 'utf-8' })
  } else if (type == "remove") {
    cards = cards.filter(card => card != roomID)
    $.log(`\n🟢房间号 ${roomID} 已删除`)
    $.setdata(JSON.stringify(cards), "@Bili.Advanced.Live.Cards")
    $.done({ body: JSON.stringify({ "Status": "Delete Success", "Current Cards": cards }), encoding: 'utf-8' })
  }
})()
