...
// 设置默认画质
current_qn = respBody.data.playurl_info.playurl.stream[0].format[0].codec[0].current_qn

accept_quality = respBody.data.playurl_info.playurl.stream[0].format[0].codec[0].accept_qn
$.log(accept_quality)
if (accept_quality.includes(10000)) {
  respBody.data.playurl_info.playurl.stream.forEach(element => {
    element.format[0].codec[0].current_qn = 10000
  });
  $.log("\n🟢已经设置 原画 画质")
}
