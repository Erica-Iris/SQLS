...
async function onResponse(body) {
    ...
    // 过滤函数
    function filterCardsByCategory(body, categories) {
        if (!body || !body.data || !body.data.card_list) {
            return [];
        }

        return body.data.card_list.filter(card => {
            const areaName = card.card_data?.small_card_v1?.area_name;
            return !categories.includes(areaName);
        });
    }
    // 进行过滤
    body.data.card_list = filterCardsByCategory(data, categoriesToFilter);
    ...
    console.log("剩余: " + body.data.card_list.length + "条数据");
    response.body = body;
    ...
    //     response.bodyFilePath = "/Users/iris/Documents/Project/SQLS/quanx/script/bilibili/a.json"
    console.log("================END==================")
    // Done
    return response;
}
